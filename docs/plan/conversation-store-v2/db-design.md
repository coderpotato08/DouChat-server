...
## 三、chat_compress_snapshots 压缩快照集合设计
### 集合用途
...
| sessionId | String | 归属会话ID，关联主集合chat_messages |
...
db.chat_messages.createIndex({ sessionId: 1, requestId: 1, sortIndex: 1 });

// 2. 压缩消息快速筛选索引，批量查询已压缩历史对话
db.chat_messages.createIndex({ sessionId: 1, isCompressed: 1 });
...
```
### chat_compress_snapshots 快照集合索引
```javascript
// 按会话+时间倒序，快速取最新压缩快照
db.chat_compress_snapshots.createIndex({ sessionId: 1, createTime: -1 });
```
