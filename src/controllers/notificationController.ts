...
export const loadFriendNotifications = async (ctx: Context) => {
  // 查询好友通知记录
  const { userId } = ctx.request.body as FriendNotificationsParams;
  try {
    const list = await FriendNotificationModel.find({ friendId: userId })
      .populate({
        path: "userId",
        model: "Users",
...
export const loadGroupNotifications = async (ctx: Context) => {
  ...
  try {
    const list = await GroupNotificationModel.find({ userId }, null, {
      lean: true,
    })
      .populate({
        path: "inviterId",
...
export const loadAllUnreadCounts = async (ctx: Context) => {
  ...
  try {
    ...
    const contactUnreadList = await Promise.all(
      userContactList.map(async (contact) => {
    ...
    const groupUnreadList = await Promise.all(
      groupContactList.map(async (contact) => {
        const { userId, groupId, createTime } = contact;
        const messages = await GroupMessageReadModel.find(
          { userId, groupId, unread: true },
          null,
          { lean: true }
        ).populate({
          path: "messageId",
