...
mainAgent.sendThinkingStreamMessage(requestId, userId, message, modelProvider, streamHandler?)
...
toolManager.executeToolHandler(requestId, userId, modelProvider, toolCallId, toolName, rawArgs): Promise<ToolExecutionResponse>
...
`safe_path`, `run_bash`, `run_read`, `run_write`, `todo`, `task_create`, `task_get`, `task_list`, `task_update`, `get_message_records`, `search_friends_markdown`

注意：`task_create`、`task_get`、`task_list`、`task_update`、`get_message_records`、`search_friends_markdown` 在 `DEFAULT_ALLOW_TOOLS` 中声明但尚未有对应工具实现注册。
